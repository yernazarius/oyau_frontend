...
export const bookingToAppointment = (booking: BookingRead): Appointment => {
	...
	return {
		...
		clientName: `${booking.client.name} ${booking.client.surname}`,
...
// API functions
export const createBooking = async (booking: BookingCreate): Promise<BookingRead> => {
	const response = await axiosInstance.post('/api/booking/bookings', booking)
	return response.data
}

export const updateBooking = async (booking: BookingUpdate): Promise<BookingRead> => {
...
export const getBooking = async (bookingId: number): Promise<BookingRead> => {
	const response = await axiosInstance.get(`/api/booking/bookings/${bookingId}`)
	return response.data
}
...
export const getBookingsByWorkspace = async (workspaceId: number): Promise<BookingRead[]> => {
	const response = await axiosInstance.get(`/api/booking/bookings/workspace/${workspaceId}`)
	return response.data
}